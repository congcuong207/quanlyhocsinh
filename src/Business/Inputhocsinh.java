...
public class Inputhocsinh {
    ...
    public static void Them(){
        ...
        System.out.println(" ----------------------------------------------------------");
        Nguoi ng=new Nguoi(Mahs,Hoten,Gioitinh,Lop,Ngaysinh,Dantoc,Diachi);//Khai báo đối tượng mới
        nguoiArrayList.add(ng);
    }
    public static void Hien1hs(Nguoi ng) //Truyền 1 đối tượng vào
    {
    ...
    public static void Hien()
    {
        System.out.println("-----------------------------------------------------------------------------------------------------------------------------------");
        System.out.printf("%20s %25s %10s %15s %10s %30s \n","Mã","Họ tên","Lớp","Ngày sinh","Dân tộc","Địa chỉ");
        for (Nguoi ng: // foreach để duyệt mảng duyệt từng thằng 1
             nguoiArrayList) {
            Hien1hs(ng);//gọi đến hàm hiện 1 người
        }
        System.out.println("-----------------------------------------------------------------------------------------------------------------------------------");
    }
    public static void Xoa(String Ma)//Nhập mã hs cần xóa
    {
        for(int i=0;i<nguoiArrayList.size();i++)
        {
            if(Ma.equalsIgnoreCase(nguoiArrayList.get(i).getMahs())) //equalsIgnoreCase = so sánh bỏ qua ký tự hoa thường
            {
    ...
    public static void Doc()
    {
        try {
            ...
            {
                String Mamh=line.split("#")[0]; //tách chuỗi bởi dấu #
